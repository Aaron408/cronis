...
app.post("/api/send-recovery-code", async (req, res) => {
  ...
  try {
    ...
    //Eliminar cualquier codigo de verificación existente de este correo.
    await pool.query("DELETE FROM verification_codes WHERE email = ?", [email]);

    //Insertar un código de verificación nuevo
    await pool.query(
      "INSERT INTO verification_codes (email, code) VALUES (?, ?)",
      [email, verificationCode]
    );
    ...
    const mailOptions = {
      ...
      text: `Tu código de recuperación para CRONIS es: ${verificationCode}. Este código expira en 3 minutos.`,
    };
    ...
    console.error(
      "Error en el proceso de envío de código de recuperación:",
      error
    );
    res.status(500).json({
      message: "Hubo un error al procesar la solicitud de recuperación.",
      error: error.message,
    });
  }
});
...
app.post("/api/verify-recovery-code", async (req, res) => {
  ...
  try {
    ...
    if (results.length > 0) {
      //Generar un token temporal para manejar la recuperación de la contraseña
      const tempToken = jwt.sign({ email }, process.env.JWT_SECRET, {
        expiresIn: "15m",
      });
      res.json({ isValid: true, tempToken });
    } else {
...
app.post("/api/reset-password", async (req, res) => {
  ...
  try {
    //Verificar el token temporal anteriormente generado
